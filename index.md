# ポートフォリオ
このページは、私が個人練習で作成したAI・画像分類・3D作品などを紹介するポートフォリオです。<br>
独学でプログラミングを学習し、業務自動化（Python/VBA/GAS）と画像系PoC（CNN/OCR/YOLO）を中心に制作しています。<br>  
生成AI（ChatGPT / Claude / Gemini）は補助的に活用し、仕様整理・コードレビュー・動作検証・修正は自分で行っています。

## 個人制作の代表プロジェクト
- 犬猫画像分類（ResNet50 + Fine-tuning / 精度98.9%）：https://github.com/suzuki2n/catdog288
- 猫の自動切り抜き（YOLOv8-seg + OpenCV / Docker）：https://github.com/suzuki2n/YOLO_cats
- 3D作品（配布）：Booth / VRoid Hub（下部にリンクあり）

# 開発アプローチ
- 企画段階：生成AIを活用して要件と完成イメージを言語化
- 実装段階：生成コードをレビューし、理解・修正しながら実装
- 改善段階：テストと不具合修正を繰り返し、目的の動作に調整

# 具体的な活用事例
- 犬猫分類プロジェクトでは、ResNet50転移学習モデルで期待した精度が得られなかった際、3つのAI（ChatGPT・Claude・Gemini）の力を借りて原因分析を行い、最終的に問題を解決して完成させました。
- 猫の切り抜きツールでは、主にChatGPTを使用してコード作成を進め、不明な点についてはClaudeから詳細な解説やアドバイスを受けて理解を深めました。

## ディープラーニング画像分類モデル（猫・犬）
- 目的：犬・猫の2分類を高精度で実現し、学習～評価の実験設計力を身につける。
- 犬猫分類AIモデル  
- 使用技術：TensorFlow, CNN, 転移学習（ResNet50 + ファインチューニング）  
- 精度：精度98.9%

このプロジェクトでは、転移学習とファインチューニングを使い、犬猫分類の精度を最大98.9%まで向上させました。<br>  
モデルやコード、実験ログ（.ipynb）も全て公開中です。<br>  
このリポジトリは、犬猫画像分類の転移学習プロジェクトの記録です。<br>  
最終的に ResNet50＋Fine-tuning により **精度98.9%** を達成しました。<br>

- このリポジトリに含まれるもの  
catdog_model_128.ipynb（CNNモデル）<br>
catdog_model_224.ipynb（CNNモデル）<br>
catdog_model_ResNet50_BATCH12_224.ipynb（実験中精度約63％のモデル）<br>
catdog_model_ResNet50_BATCH16_224.ipynb（実験中精度約63％のモデル）<br>
catdog_model_VGG16_BATCH8_224.ipynb（VGG16モデル精度93％）<br>
catdog_model_VGG16_BATCH16_224.ipynb（VGG16モデル精度93％）<br>

**完成モデル**  
catdog_model_ResNet50_BATCH16_FT2_224.ipynb（精度98.9％）<br>

img_resize.py（データセット用画像の前処理ツール）<br>
mkdir_learning.py（データセット用フォルダの作成ツール）<br>
perform_classification.py（モデルを実際の画像で実行、画像を分類保存するツール）<br>


## 猫の自動切り抜き（YOLOv8-seg + OpenCV）
### 概要
- 目標：大量の画像から猫だけを抽出し、透過PNGを自動生成
- 学習目的：YOLOの実装手順とDockerの使い方を習得
- 技術：Ultralytics YOLOv8-seg（推論のみ / 学習なし）＋ OpenCV。
- システム構成：前処理→推論→切り抜き→QCの完全バッチ処理
- 開発環境：Docker、ubuntu
- 品質管理：仕上がり判定のためのQCツールを実装

### 学習内容
- WSL2 + Docker + GPU での環境構築・運用
- VS Code の Dev Containers でコンテナ内での開発・実行
- 画像前処理（縮小/拡大、CLAHE、軽いシャープ）

### 手順
- preprocess.py：小さい画像を拡大、低コントラストを CLAHE で補正
- cutout_batch.py：YOLOv8-seg 推論 → マスク合成 → 透過 PNG 保存（穴が大きい/マスク無は GrabCut 保険）
- qc_cutouts.py：品質判定をする。面積・端接触・穴・連結成分で ok / review を自動仕分け

#### 使用ファイル
![使用ファイル](assets/img/yolo/image.png)
#### 使い方
![使い方](assets/img/yolo/image-6.png)

### 残された課題と今後の改善点
#### 低コントラスト画像への対応
- 課題：白猫×白背景などの低コントラスト画像でYOLO抽出時に猫の一部が欠ける
- 原因：境界の判別が困難なため、セグメンテーション精度が低下
#### 複数オブジェクトの処理
- 課題：多頭飼いや猫が重なった画像で抽出が不完全になる
- 影響：一部が欠けた状態での切り抜きが発生
#### エッジ処理の精度向上
- 課題：切り抜き境界でギザつきや背景の混入が発生
- 根本原因：YOLOは正しく猫を検出しているが、境界判別の限界により切り抜き品質が低下
- 連鎖的影響：不正確な切り抜きによりQCツールでの画像振り分けも不安定化

#### 今後の方向性
- 前処理段階でのコントラスト強化（試行済みだが十分な改善には至っていない）
- 後処理でのエッジ平滑化
- より高精度なセグメンテーションモデルの検討

### 切り抜き結果
![切り抜き結果](assets/img/yolo/image-7.png)
- 飼い猫の画像

### ライセンス・権利
- モデル：Ultralytics を pip 依存／Docker 内で利用（AGPL への配慮として本体配布はしない）。
- 画像：Kaggle 画像は学習・検証のみ。公開画像は、自撮りした飼い猫の画像のみを使用しています。

順次プロジェクト内容を整理し、追加していきます。<br>

# 3D制作実績
Blender、Photoshop、Substance Painter、Unityを使用し、3Dモデルを制作してきました。
アバター、小物、ワールド制作の経験があり、用途に応じた調整や、公開用データの整備にも対応していました。

## 企業向け制作実績
VRイベント「Virtual Market」にて、企業ブース制作やプロップ制作を担当しました。  
Virtual Market は、VRChat 上で開催される大規模なバーチャルマーケットイベントです。

### Vket5 企業ブース

支給資料をもとに、指示に沿って制作しました。  
限られた広さの中で、アバターが問題なく螺旋階段を上れることと、資料通りの見た目を両立できるよう、各パーツのサイズやバランスを調整しました。
<div style="display:flex; gap:16px; flex-wrap:wrap; justify-content:center; align-items:flex-start; margin:16px auto 24px;">
  <img src="assets/img/3D/pluto.png" alt="企業ブース作成：pluto"
       style="width:48%; max-width:420px; height:auto; display:block; border-radius:8px;">
 </div>

### Vket6 企業ブース

支給資料をもとに、指示に沿って制作しました。  
商品のモデリングは現物がなかったため、支給された写真や公式サイトの情報をもとに形状を再現しました。  
また、配布用ヘッドセットにはボーンを設定し、アバターの頭部に合わせて調整できるようにしています。

<div style="display:flex; gap:16px; flex-wrap:wrap; justify-content:center; align-items:flex-start; margin:16px auto 24px;">
  <img src="assets/img/3D/ロジクール.png" alt="企業ブース作成：ロジクール"
       style="width:48%; max-width:420px; height:auto; display:block; border-radius:8px;">
  <img src="assets/img/3D/ロジクール2.png" alt="企業ブース作成：ロジクール"
       style="width:48%; max-width:420px; height:auto; display:block; border-radius:8px;">
</div>
<div style="display:flex; gap:16px; flex-wrap:wrap; justify-content:center; align-items:flex-start; margin:16px auto 24px;">
  <img src="assets/img/3D/ヘッドセット.png" alt="企業ブース作成：ロジクール"
       style="width:48%; max-width:420px; height:auto; display:block; border-radius:8px;">
  <img src="assets/img/3D/マウス.png" alt="企業ブース作成：ロジクール"
       style="width:48%; max-width:420px; height:auto; display:block; border-radius:8px;">
</div>

### Vket2021 企業ブース

ブース本体に加え、約20個のプロップを制作しました。  
作り込むべき箇所と、容量を抑えるために簡略化すべき箇所を見極め、クオリティと軽量性のバランスを意識して制作しました。

<div style="display:flex; gap:16px; flex-wrap:wrap; justify-content:center; align-items:flex-start; margin:16px auto 24px;">
  <img src="assets/img/3D/玄人志向.png" alt="企業ブース作成：玄人志向"
       style="width:48%; max-width:420px; height:auto; display:block; border-radius:8px;">
 </div>

### Vket2022 Summer 会場モデル制作

大阪およびニューヨーク会場で、複数の背景・オブジェクト制作を担当しました。  
大阪ではロッテリアとその周辺ビル、川沿いのビル群、自転車の改変、くいだおれ太郎などを制作しました。  
ニューヨークではタイムズスクエアの階段、銅像の台座、出展企業である「ベルク」様のマスコットキャラクターなどをモデリングしました。

<div style="display:flex; gap:16px; flex-wrap:wrap; justify-content:center; align-items:flex-start; margin:16px auto 24px;">
  <img src="assets/img/3D/制作実績.png" alt="大阪"
       style="width:48%; max-width:420px; height:auto; display:block; border-radius:8px;">
  <img src="assets/img/3D/制作実績2.png" alt="ベルク"
       style="width:48%; max-width:420px; height:auto; display:block; border-radius:8px;">
</div>

### 猫モデル
- 種類：動物モデル
- 工程：モデリング、ボーン設定、UV調整、テクスチャ差し替え対応
- 仕様：△13000 / マテリアル3個 / テクスチャサイズ 512～2048

猫の柔らかい動きを表現できるよう、回転と移動が可能なボーンを設定しました。
UVは全猫で共通化し、テクスチャを差し替えることで毛並みの変更に対応できる構成にしています。
歯の色や形も1本ずつ調整し、細部の見た目にも配慮しました。

<div style="display:flex; gap:16px; flex-wrap:wrap; justify-content:center; align-items:flex-start; margin:16px auto 24px;">
  <img src="assets/img/3D/かわいいねこ.png" alt="VRChat向け3Dモデル"
       style="width:48%; max-width:420px; height:auto; display:block; border-radius:8px;">
  <img src="assets/img/3D/MMDねこ.png" alt="猫モデル詳細"
       style="width:48%; max-width:420px; height:auto; display:block; border-radius:8px;">
</div>

### 3D小道具：琴
- フォーマット：FBX形式
- 使用シェーダー：Standard
- 公開：Boothで無料配布
▶ [配布ページはこちら](https://suzuki2n.booth.pm/items/3971677)
<br>

### VRMアバター<br>
VRoid Hubにて公開中<br>
▶ [VRMモデルを見る](https://hub.vroid.com/characters/6889964990498381032/models/4232243861661725226)
<br>

